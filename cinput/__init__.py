from typing import AnyStr, Any, Iterable, List, Callable, Union, Type, Dict, Tuple
...
def keyValidator(keys:Iterable[str]):
    def validator(value:str):
        return value in keys
    return validator

def menuArgsInput(menu:Menu, name:str):
    stk = []
    
    def argValidator(keys:Iterable[str]):
        def validator(value:Tuple[str, List[str]]):
            key, _ = value
            return keyValidator(keys)(key)
        return validator
    
    def argParser(value:str):
        args = value.split(' ')
        val = args.pop(0)
        return True, (val, args)
    ...
    while True:
        ...
        if isinstance(menu, dict):
            ...
            for k in menu.keys():
                keys.append(k)
            m, args = cinput("Enter value: ", Tuple[str, List[str]], argValidator(keys), opt, argParser)
            if m == 'back':
                if len(stk) == 0:
                    return
                menu = stk.pop()
